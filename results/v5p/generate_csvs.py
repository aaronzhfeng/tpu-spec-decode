#!/usr/bin/env python3
"""Generate CSV result tables from v5p benchmark JSON logs.

Reads from:
  - Standalone benchmark JSON: results/v5p/standalone_*.json
  - vLLM pipeline JSON:       /dev/shm/*/bench_math_*/summaries/
                             (tries dflash-test-outputs, elephant-dflash-outputs)

GPU paper numbers are from DFlash paper Table 1 (Qwen3-4B, temperature=0).

Usage:
    python results/v5p/generate_csvs.py
    DFLASH_OUTPUT_DIR=/dev/shm/elephant-dflash-outputs python results/v5p/generate_csvs.py
"""
...
# v5p may use user-specific output dir when /dev/shm/dflash-test-outputs not writable
OUTPUT_BASE_DIRS = [
    os.environ.get("DFLASH_OUTPUT_DIR", ""),
    "/dev/shm/dflash-test-outputs",
    "/dev/shm/elephant-dflash-outputs",
]
...
def find_vllm_json():
    """Find the most recent vLLM bench_math run (checks multiple output dirs)."""
    for base in OUTPUT_BASE_DIRS:
        if not base or not os.path.isdir(base):
            continue
        pattern = os.path.join(base, "bench_math_*", "summaries", "overall.json")
        paths = sorted(glob.glob(pattern))
        if paths:
            latest = paths[-1]
...
def find_eagle3_json():
    for base in OUTPUT_BASE_DIRS:
        if not base or not os.path.isdir(base):
            continue
        pattern = os.path.join(base, "bench_custom_*", "summaries", "overall.json")
        paths = sorted(glob.glob(pattern))
        for p in paths:
            with open(p) as f:
                data = json.load(f)
...
def main():
    print("=" * 60)
    print("Generating CSV tables from v5p benchmark run logs")
    print("=" * 60)
    print()
